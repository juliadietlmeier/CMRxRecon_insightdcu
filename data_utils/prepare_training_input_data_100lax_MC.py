...
for d in dirs:# walk over patient directories
    ...
    for fnames in files_d:
        ...
        arr = (ff['kspace_full'])
        shape=np.shape(arr)#[12,3,sc,204,448]
        if idt=='lax':
            n_slices=shape[1]
            timeframe=shape[0]
            sc=shape[2]
            ...
            for t in range(0,timeframe):
                for k in range(0,n_slices):
                    for s in range(0,sc):
                        ...
                        f_name='FS_'+d+'_'+idt+'_'+str(t)+'_'+str(k)+'_'+str(s)+'.jpg'
            ...
            for t in range(0,timeframe):
                for k in range(0,n_slices):
                    for s in range(0,sc):
                        ...
                        f_name='AC_'+d+'_'+idt+'_'+str(t)+'_'+str(k)+'_'+str(s)+'.jpg'
...
#------------------------------------------------------------------------------
"=============================================================================="
"=== now place all AccFactors images into one train_lax_ALL folder ============"

path, dirs, files = next(os.walk(dst_input_AC10))# there are less AC files than FS for some reason
...
dst_train_target_FS='/home/daa/Desktop/CMRxRecon_challenge_MICCAI23/MultiCoil/Cine/Training_Set_julia/train_lax_ALL/target_FS/'
dst_train_input_AC='/home/daa/Desktop/CMRxRecon_challenge_MICCAI23/MultiCoil/Cine/Training_Set_julia/train_lax_ALL/input_AC/'

f_list=os.listdir(src_input_AC10)# ['FS_P001_2.jpg', 'FS_P001_0.jpg', 'FS_P001_1.jpg', ....]
...
for k in range(0,len(f_list_train)):
    ...
    cv2.imwrite(dst_train_input_AC+f_list_train[k], img)
    print(f_list_train[k])
...
path, dirs, files = next(os.walk(src_target_FS))
for k in range(0,len(f_list_train)):
    img=cv2.imread(path+f_list_train[k].replace('AC','FS'))
    cv2.imwrite(dst_train_target_FS+f_list_train[k].replace('AC','FS'), img)
    print(f_list_train[k].replace('AC','FS'))
