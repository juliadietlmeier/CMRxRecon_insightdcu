...
def reconstruct_sc_image_lax(path, accfactor):
...
# load model once before the loops --------------------------------------------
    ...
    state_dict = torch.load("/home/daa/Desktop/CMRxRecon_challenge_MICCAI23/Python_submission/denoisingUNET_300epochs_lax_GNA_ALL_DA.pt")
...
#------------------------------------------------------------------------------
    ...
    for t in range(0,timeframe):
        for k in range(0,sz):
            ...
            #recon_model = build_unet()
            #state_dict = torch.load("/home/daa/Desktop/CMRxRecon_challenge_MICCAI23/Python_submission/denoisingUNET_300epochs_lax_GNA_ALL.pt")
            #recon_model.load_state_dict(state_dict)
            #device=get_device()
            #recon_model = recon_model.to(device)
